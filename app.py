...
.metric-label { color: #94a3b8; font-size: 0.875rem; text-transform: uppercase; letter-spacing: 0.05em; font-weight: 600; }
...
<div class="hero">
    ...
    <div style="margin-top: 1rem;">
        <span style="background: rgba(74,222,128,0.15); color: #4ade80; border: 1px solid rgba(74,222,128,0.3);
...
# ============================================================
# ✨ Floating Action Buttons — Sidebar Toggle + Fullscreen
# ============================================================
st.markdown("""
<div class="fab-container">

    <!-- Sidebar toggle -->
    <button class="fab-btn" onclick="toggleSidebar()" title="Toggle Sidebar">
    ...
    <!-- Fullscreen toggle -->
    <button class="fab-btn" id="fab-fs" onclick="toggleFullscreen()" title="Toggle Fullscreen">
        <!-- expand icon (default) -->
...
(function () {
    ...
    window.toggleSidebar = function () {
        ...
        // Try the native Streamlit collapse button first
        var btn =
            ...
            pd.querySelector('section[data-testid="stSidebar"] ~ div button');
    ...
    window.toggleFullscreen = function () {
        var pd  = window.parent.document;
        var exp = document.getElementById('fs-expand');
        var cmp = document.getElementById('fs-compress');
        ...
        if (!pd.fullscreenElement) {
            pd.documentElement.requestFullscreen()
                ...
                .catch(function (err) { console.warn('Fullscreen error:', err); });
        } else {
            pd.exitFullscreen()
                .then(function () {
...
with st.sidebar:
    ...
    location_display = st.session_state.selected_zone if location_mode == "Manual Select" \
    ...
    data_source = st.radio(
        ...
        index=["Automatic (API)", "Sample Data", "Upload CSV (Admin)"].index(st.session_state.data_source)
              if st.session_state.data_source in ["Automatic (API)", "Sample Data", "Upload CSV (Admin)"] else 0,
        help="Automatic fetches real-time data from Electricity Maps. Sample uses synthetic data."
    )
    ...
    deadline = st.text_input("Deadline (HH:MM)",        value=st.session_state.deadline,
                              help=f"Latest finish time ({st.session_state.timezone})")
    ...
    available_mins = (deadline_mins - now_mins) if deadline_mins > now_mins \
                     else (1440 - now_mins + deadline_mins)
...
# ============================================================
@st.cache_data(ttl=300)
def load_data_auto(lat, lon, api_token=None):
    ...
    if not sample_file.exists():
        ...
        for h in range(24):
            for m in [0, 15, 30, 45]:
                ...
                thermal = int(base_load * lf * 0.6)
                hydro   = 1500
                nuclear = 2000
                res     = int(800 + (1200 if 10 <= h <= 16 else 0))
                data.append({"time": t, "thermal_mw": thermal, "hydro_mw": hydro,
                             "nuclear_mw": nuclear, "res_mw": res})
        pd.DataFrame(data).to_csv(sample_file, index=False)

    return pd.read_csv(sample_file)

def load_data_uploaded(file):
    try:
        df = pd.read_csv(file)
...
with kpi_cols[0]:
    ...
    <div class="kpi">
        ...
        <div class="metric-value" style="font-size:1.2rem;">
            {st.session_state.selected_zone if st.session_state.location_mode == "Manual Select" else "Auto-Detected"}
        </div>
        <div style="color:#64748b;font-size:0.75rem;">{st.session_state.timezone}</div>
    </div>
    """, unsafe_allow_html=True)
...
with kpi_cols[1]:
    ...
    <div class="kpi">
        ...
        <div style="color:#64748b;font-size:0.75rem;">Live ({st.session_state.data_source.split(' ')[0]})</div>
    </div>
    """, unsafe_allow_html=True)
...
with kpi_cols[2]:
    ...
    <div class="kpi">
        ...
        <div style="color:#64748b;font-size:0.75rem;">{kw:.2f} kW • {ceil_to_step(duration)} min</div>
    </div>
    """, unsafe_allow_html=True)
...
with kpi_cols[3]:
    ...
    <div class="kpi" style="border-left-color:{ci_color}">
        ...
        <div style="color:#64748b;font-size:0.75rem;">{get_ci_status(current_ci)} intensity</div>
    </div>
    """, unsafe_allow_html=True)
...
with kpi_cols[4]:
    ...
    <div class="kpi" style="border-left-color:{best_color}">
        ...
        <div style="color:#64748b;font-size:0.75rem;">
            {f"Save ~{potential_savings:.1f}%" if potential_savings > 0 else "No savings"}
        </div>
    </div>
    """, unsafe_allow_html=True)
...
with tab1:
    ...
    with dash_cols[0]:
        ...
        if windows:
            for i, w in enumerate(windows[:3]):
                fig.add_vrect(x0=w["start"], x1=w["end"],
                              fillcolor=w["color"], opacity=0.25,
                              line_width=2 if i == 0 else 1, line_color=w["color"], layer="below")
        ...
        fig.add_annotation(x=now_time,  y=max_ci_val, text="NOW",      showarrow=False, font=dict(color="#4ade80", size=11))
        fig.add_annotation(x=deadline,  y=max_ci_val, text="DEADLINE", showarrow=False, font=dict(color="#f87171", size=11))
    ...
    with dash_cols[1]:
        ...
        fig2 = go.Figure(data=[go.Pie(
            labels=mix_data["Source"], values=mix_data["MW"],
            hole=0.65, marker_colors=mix_data["Color"],
                ...
                with col:
                    ...
                    <div class="card" style="{border_style}">
                        <div style="text-align:center;margin-bottom:0.5rem;">
                            <span style="background:{'#4ade80' if is_best else '#374151'};
                                         color:{'#0f172a' if is_best else 'white'};
                        ...
                        <div style="background:rgba(74,222,128,0.1);padding:0.4rem;border-radius:6px;
...
with tab3:
    ...
    <div style="background:#1e293b;padding:1rem;border-radius:8px;margin-bottom:1rem;">
        ...
        <div style="color:white;font-size:1rem;">
            {st.session_state.selected_zone if st.session_state.location_mode == "Manual Select"
    ...
    fig_comp = go.Figure()
    fig_comp.add_trace(go.Bar(name='Baseline (Now)',    x=['CO₂ Emissions'], y=[co2_now],
                              marker_color='#f87171', text=[f'{co2_now:.3f} kg'], textposition='outside', width=0.35))
    fig_comp.add_trace(go.Bar(name='Optimized (Best)', x=['CO₂ Emissions'], y=[co2_best],
                              marker_color='#4ade80', text=[f'{co2_best:.3f} kg'], textposition='outside', width=0.35))
...
with tab4:
    ...
    with st.form("run_form"):
        ...
        with form_cols[1]:
            default_start = windows[st.session_state.selected_window]["start"] \
                            if windows and st.session_state.selected_window < len(windows) else now_time
        ...
        if submitted:
            ...
            if kwh_used <= 0:
                ...
                row = {
                    ...
                    "location_zone":   st.session_state.selected_zone if st.session_state.location_mode == "Manual Select"
                                       else f"{st.session_state.lat:.2f},{st.session_state.lon:.2f}",
